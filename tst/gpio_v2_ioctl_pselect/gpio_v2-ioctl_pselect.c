...
#define INPUTTIMEOUT     1e8          /* pselect input timeout (ns) */
...
/**
 * @brief simple wrapper for pselect to monitor a single file descriptor.
 ...
 */
int pselect_timer_fd (int fd, unsigned sec, unsigned nsec)
{
  fd_set set;
  int res;
  ...
  /* watch fd with timeout_sec timeout */
  res = pselect (fd + 1, &set, NULL, NULL, &ts, NULL);
...
void *threadfn_reader (void *data)
{
  ...
  while (writing == 1) {
    ...
    int nfds = pselect_timer_fd (linereq->fd, 0, INPUTTIMEOUT),
        nbytes = 0;

    /* validate poll return */
    if (nfds < 0) {   /* error */
      perror ("pselect_linereq->fd");
      continue;
    }

    if (nfds == 0) {  /* timeout occurred */
#ifdef DEBUG
      fputs ("pselect - timeout", stderr);
#endif
      writing = 0;
      break;
    }

    if ((nbytes = read (linereq->fd, &lineevent, sizeof lineevent)) == -1) {
  ...
  gpio_line_close_fd (gpio);  /* close linereq file descriptor */
