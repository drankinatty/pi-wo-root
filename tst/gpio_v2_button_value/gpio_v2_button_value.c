...
void *threadfn_read_btn (void *data)
{
  ...
  /* gpio_v2_line_values struct for reading gpio line value */
  struct gpio_v2_line_values linevals = { .bits = 0x00, .mask = 0x01 };
  ...
  while (monitoring == 1) {
    ...
    if (readfds.revents & POLLIN) {   /* process event */
      ...
      if (lineevent.id == GPIO_V2_LINE_EVENT_RISING_EDGE) {
        ...
        count_rising += 1;
        /* get value and confirm with PULL_UP, ACTIVE_LOW, the "active",
         * "rising" edge is the edge going from line-voltage to zero and
         * confirm the line value is 1 (Hi) for zero voltage.
         */
        if (gpio_line_get_values (linereq, &linevals, linevals.mask) == -1) {
          fputs ("error: get values rising edge\n", stderr);
          monitoring = 0;
          break;
        }
        printf ("rising edge  : %3llu   (value: %llu)\n",
                count_rising, linevals.bits & 0x01u);
      }
      else if (lineevent.id == GPIO_V2_LINE_EVENT_FALLING_EDGE) {
        event_ts_end = lineevent.timestamp_ns;
        count_falling += 1;
        /* get value and confirm with PULL_UP, ACTIVE_LOW, the "inactive",
         * "falling" edge is the edge going from zero to line-voltage and
         * confirm the line value is 0 (Lo) for line voltage.
         */
        if (gpio_line_get_values (linereq, &linevals, linevals.mask) == -1) {
          fputs ("error: get values falling edge\n", stderr);
          monitoring = 0;
          break;
        }
        printf ("falling edge : %3llu   (value: %llu, press duration : %llu ms)\n",
                count_falling, linevals.bits & 0x01u,
                (event_ts_end - event_ts_start) / 1000000);
      }
      else {
        count_other += 1;
      }
