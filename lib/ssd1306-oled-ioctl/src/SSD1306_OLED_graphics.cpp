...
*/
OLED_Return_Codes_e SSD1306_graphics::drawChar (int16_t x, int16_t y,
  ...
  if (character < _CurrentFontoffset ||
      character >= (_CurrentFontLength +  _CurrentFontoffset))
  {
    printf ("Error drawChar 3: Character out of Font bounds: %u:  %u  %u<->%u \r\n",
            OLED_CharFontASCIIRange, character,_CurrentFontoffset,
            (_CurrentFontLength + _CurrentFontoffset));
  ...
  for (int8_t i = 0; i < (_CurrentFontWidth + 1); i++ ) {
    ...
    if (i == _CurrentFontWidth) {
      ...
      switch (_FontNumber) {
        case OLEDFont_Default:
          line = pFontDefaultptr[((character - _CurrentFontoffset) *
                                  _CurrentFontWidth) + i];
          break;
        case OLEDFont_Thick:
          line = pFontThickptr[((character - _CurrentFontoffset) *
                                _CurrentFontWidth) + i];
          break;
        case OLEDFont_SevenSeg:
          line = pFontSevenSegptr[((character - _CurrentFontoffset) *
                                   _CurrentFontWidth) + i];
          break;
        case OLEDFont_Wide:
        ...
        case OLEDFont_Tiny:
          ...
          break;
        case OLEDFont_Homespun:
          line = pFontHomeSpunptr[((character - _CurrentFontoffset) *
                                   _CurrentFontWidth) + i];
          break;
        default:
          printf ("Error drawChar 1: Wrong font selected, must be font 1-6 : %u\r\n",
